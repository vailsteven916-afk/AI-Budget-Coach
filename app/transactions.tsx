...
export default function Transactions() {
  ...
  return (
    <View className="flex flex-col min-h-full pb-24">
      <View className="px-6 pt-12 pb-6 flex flex-row items-center gap-4 sticky top-0 bg-gray-50/80 dark:bg-zinc-950/80 backdrop-blur-md z-10">
        <TouchableOpacity onPress={() => router.back()} className="p-2 -ml-2 text-zinc-500 bg-white dark:bg-zinc-900 rounded-full shadow-sm">
        ...
        <View className="bg-white dark:bg-zinc-900 rounded-3xl p-2 shadow-sm border border-zinc-100 dark:border-zinc-800">
          {transactions.length === 0 ? (
            ...
            transactions.map((t, i) => {
              ...
              return (
                <View key={t.id} className={cn("flex flex-row items-center justify-between p-4", i !== transactions.length - 1 && "border-b border-zinc-100 dark:border-zinc-800")}>
